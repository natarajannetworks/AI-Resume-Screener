...
def call_mistral_api(prompt: str, retries: int = 3) -> str:
    """Standardized API caller with real retry + backoff on rate limits / server errors."""
    ...
    for attempt in range(retries + 1):
        try:
            ...
            # Retryable: rate limit or server-side hiccup
            if response.status_code in (429, 500, 502, 503, 504) and attempt < retries:
                wait = 4 * (attempt + 1)  # 4s, 8s, 12s
                print(f"  🕒 Status {response.status_code}. Retrying in {wait}s... (attempt {attempt + 1}/{retries})")
                time.sleep(wait)
                continue

            # Out of retries or non-retryable status
            print(f"❌ API Error: HTTP {response.status_code} - {response.text[:200]}")
            return json.dumps({"error": f"AI Service Busy or Error (HTTP {response.status_code})."})

        except requests.exceptions.Timeout:
            if attempt < retries:
                wait = 4 * (attempt + 1)
                print(f"  🕒 Request timed out. Retrying in {wait}s... (attempt {attempt + 1}/{retries})")
                time.sleep(wait)
                continue
            return json.dumps({"error": "AI request timed out after retries."})
    ...
    return json.dumps({"error": "Unknown failure after retries"})
